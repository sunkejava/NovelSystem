...
async function loadBase(){
  [detail.value,voiceProfiles.value]=await Promise.all([novelApi.detail(id),voiceApi.options()]);
}
async function loadScripts(){
  const r=await novelApi.scripts(id,scriptQuery.value);
  scripts.value=r.items;scriptTotal.value=r.total;scriptSpeakers.value=r.speakers||[];
}
async function loadAudio(){
  audioInfo.value=await audioApi.list(id,audioQuery.value);
}
async function load(){await Promise.all([loadBase(),loadScripts(),loadAudio()]);}
...
onMounted(async()=>{await load();audioTimer=window.setInterval(()=>{if(activeTab.value==='audio')loadAudio();},4000);});
      ...
      <article v-for="character in detail.characters" :key="character.id" class="character-card">
        ...
        <div class="character-main">
          ...
          <el-select v-model="character.voiceProfileId" placeholder="绑定音色档案" clearable filterable @change="saveCharacter(character)">
            <el-option v-for="voice in voiceProfiles" :key="voice.id" :label="voice.name+' · '+voice.language" :value="voice.id"><span>{{voice.name}}</span><span style="float:right;color:var(--muted);font-size:11px">{{voice.status}}</span></el-option>
          </el-select>
        </div>
      </article>
    </div>
