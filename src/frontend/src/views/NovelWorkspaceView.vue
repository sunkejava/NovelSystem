...
const detail=ref<any>(null);const voiceProfiles=ref<any[]>([]);const activeTab=ref('characters');const working=ref(false);
const characterCount=computed(()=>detail.value?.characters?.length||0);
const scriptCount=computed(()=>detail.value?.scripts?.length||0);

async function load(){
  [detail.value,voiceProfiles.value]=await Promise.all([novelApi.detail(id),voiceApi.list()]);
}
...
<template>
<div v-if="detail">
  <PageHeader eyebrow="NOVEL WORKSPACE" :title="detail.novel.title" description="人物理解、脚本拆解、音色档案绑定与有声书生产的统一工作台。">
    <el-button class="ghost-button" :loading="working" @click="analyze">AI 重新解析</el-button>
    <el-button class="neon-button" :loading="working" @click="audio">生成完整音频</el-button>
  </PageHeader>
  ...
  <div class="workspace-stats">
    ...
    <div class="mini-stat"><span>SOURCE</span><b>{{detail.novel.sourceFile}}</b></div>
  </div>
