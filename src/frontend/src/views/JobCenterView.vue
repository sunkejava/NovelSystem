...
const jobs=ref<any[]>([]);
const total=ref(0);
const summary=ref<any>({running:0,queued:0,completed:0,failed:0});
...
const taskTypes=['AnalyzeNovel','GenerateAudio','GenerateAudioSegment','MergeAudio','LearnWritingStyle','GenerateNovel'];
...
<div class="page-fill job-page">
  ...
  <div class="task-summary-grid task-summary-grid-four">
    <div class="mini-stat"><span>运行中</span><b>{{summary.running||0}}</b></div>
    <div class="mini-stat"><span>排队中</span><b>{{summary.queued||0}}</b></div>
    <div class="mini-stat"><span>已完成</span><b>{{summary.completed||0}}</b></div>
    <div class="mini-stat"><span>失败</span><b>{{summary.failed||0}}</b></div>
  </div>
  ...
  <section class="glass-panel content-card table-page-card">
    ...
    <el-table :data="jobs" class="cyber-table" height="100%">
      ...
      <el-table-column label="状态 / 队列" width="150"><template #default="{row}"><StatusBadge :status="row.status"/><div v-if="row.status==='Queued'&&row.queuePosition" class="queue-position">队列第 {{row.queuePosition}} 位</div></template></el-table-column>
